...
CREATE TABLE
    `review` (
        ...
        KEY `review_prod_fk` (`product_id`),
        KEY `review_prod_var_fk_idx` (`prod_var_id`),
        CONSTRAINT `review_prod_fk` FOREIGN KEY (`product_id`) REFERENCES `product` (`product_id`),
        CONSTRAINT `review_prod_var_fk` FOREIGN KEY (`prod_var_id`) REFERENCES `product_variant` (`prod_var_id`),
        CONSTRAINT `review_user_fk` FOREIGN KEY (`user_id`) REFERENCES `user` (`user_id`),
        CONSTRAINT `review_chk_1` CHECK ( (
                _utf8mb4 'rating' between 0 and 5
            )
        )
